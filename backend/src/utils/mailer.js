...
// Fail loudly at boot if SMTP creds are wrong, instead of silently dropping emails later.
...
export async function sendOtpEmail({ to, name, otp, purpose }) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
      ...
      <div style="font-size: 32px; font-weight: bold; letter-spacing: 6px; background:#f3f4f6; padding: 16px; text-align: center; border-radius: 6px; margin: 16px 0;">
        ${otp}
      </div>
